...
ui <- fluidPage(
  ...
  tags$head(
    tags$script(HTML("
      Shiny.addCustomMessageHandler('saeToggleBtn', function(msg) {
  ...
  div(id = "main_app",
    ...
    sidebarPanel(
      ...
      tags$ul(
        ...
        tags$li(tags$code("docs/instructions/EU_SAE_Download_Instructions_5_2_0_rc_6_wizard_5_7.pdf")),
        tags$li(tags$code("docs/instructions/EU_SAE_User_Guide_5_2_0_rc_6_wizard_5_7.pptx")),
      ...
      actionButton("run_btn",   "2. Run Analysis",         class = "btn-primary",
                   style = "margin-right: 8px;"),
      actionButton("stop_btn",  "Stop",                    class = "btn-danger",
                   disabled = "disabled",
                   title = "End the running analysis")
    ),
...
server <- function(input, output, session) {
  ...
  # The analysis runs in a separate R process (scripts/run_pipeline_bg.R,
  # started by sae_bg_launch()). This process only launches it, polls its
  # log and status file, and finishes the dashboard state when it ends, so
  # the UI stays responsive and "Stop" can end the run.
  bg_run <- reactiveValues(
    active = FALSE, launched = FALSE, run_dir = NULL, run_dir_abs = NULL,
    run_log_path = NULL, log_lines_read = 0L, progress = NULL,
    step_counter = 0L, n_steps = 1L, phase = NULL, ctx = list(),
    started_at = NULL, stop_requested_at = NULL, last_alive_check = NULL
  )

  append_log_raw <- function(lines) {
  ...
  set_run_buttons <- function(running) {
    session$sendCustomMessage("saeToggleBtn", list(id = "run_btn",  disabled = isTRUE(running)))
    session$sendCustomMessage("saeToggleBtn", list(id = "stop_btn", disabled = !isTRUE(running)))
  }

  bg_advance <- function(stage, detail = NULL) {
    pr <- isolate(bg_run$progress)
    if (is.null(pr)) return(invisible(NULL))
    bg_run$step_counter <- isolate(bg_run$step_counter) + 1L
    n <- isolate(bg_run$n_steps)
    counter <- min(isolate(bg_run$step_counter), n)
    pr$set(value = counter, message = sprintf("[%d/%d] %s", counter, n, stage), detail = detail)
  }

  bg_close_progress <- function(detail) {
    pr <- isolate(bg_run$progress)
    if (is.null(pr)) return(invisible(NULL))
    tryCatch({
      pr$set(value = isolate(bg_run$n_steps), detail = detail)
      pr$close()
    }, error = function(e) NULL)
    bg_run$progress <- NULL
  }

  # Output inventory shown on the Outputs tab (same list as before).
  run_output_files <- c(
  ...
  run_output_descriptions <- function(do_benchmark) {
    bench_phrase <- if (isTRUE(do_benchmark)) {
  ...
  update_output_table <- function(do_benchmark, archive_dir = NULL) {
    ...
    output_rows(data.frame(
      ...
      Description = run_output_descriptions(do_benchmark),
  ...
  # Enrich the diagnostics shown in the app from the pipeline's output files
  # and regenerate the analysis brief (unchanged logic, run once at the end).
  enrich_diagnostics_after_run <- function(ctx) {
    pipeline_out <- read_pipeline_outputs()
    dd <- diagnostics_data()
    if (is.null(dd)) return(invisible(NULL))
    ...
    ufh_bench <- add_benchmark_metadata(
      ufh_bench,
      source = ctx$benchmark_source_label,
      level_label = ctx$benchmark_level_label,
      level_variable = ctx$benchmark_level_var,
      enabled = isTRUE(ctx$do_benchmark)
    )
    ...
    br <- generate_analysis_brief(
      ...
      model_type      = if ("UFH" %in% (ctx$steps %||% character())) "UFH" else "MFH",
  ...
  # Bring the remaining run.log lines into the dashboard log pane.
  bg_sync_log <- function() {
    path <- isolate(bg_run$run_log_path)
    if (is.null(path) || !file.exists(path)) return(invisible(NULL))
    lines <- tryCatch(readLines(path, warn = FALSE, encoding = "UTF-8"),
                      error = function(e) character())
    already <- isolate(bg_run$log_lines_read)
    if (length(lines) > already) {
      append_log_raw(lines[(already + 1L):length(lines)])
      bg_run$log_lines_read <- length(lines)
    }
    invisible(NULL)
  }

  # Called once when the background process has reached a final state
  # ("finished", "failed" or "stopped") or has disappeared.
  finish_background_run <- function(st) {
    bg_sync_log()
    ctx         <- isolate(bg_run$ctx)
    run_dir     <- isolate(bg_run$run_dir)
    run_dir_abs <- isolate(bg_run$run_dir_abs)
    state       <- st$state %||% "failed"
    ok          <- identical(state, "finished")
    report_missing <- ok && isTRUE(st$report_missing)
    report_reason  <- if (report_missing) st$report_reason else NULL
    archive_dir <- if (ok) st$archive_dir else NULL

    bg_advance("Finalizing", if (ok) "Checking outputs" else if (identical(state, "stopped")) "Stopped" else "Pipeline failed")
    status(if (ok) "Finalizing..." else if (identical(state, "stopped")) "Stopped by user" else "Run failed")
    update_output_table(ctx$do_benchmark, archive_dir)

    if (ok) {
      tryCatch(enrich_diagnostics_after_run(ctx), error = function(e) {
        append_log(paste("WARNING: Could not enrich diagnostics from the outputs:", conditionMessage(e)))
      })
      run_location(paste(
        paste("Run folder:", run_dir_abs),
        paste("Archived outputs:", if (!is.null(archive_dir)) normalizePath(archive_dir, winslash = "/", mustWork = FALSE) else "not created (see log)"),
        if (report_missing) {
      ...
      if (report_missing) {
        ...
        bg_close_progress("Complete (report skipped)")
      } else {
        status("Completed successfully")
        bg_close_progress("Complete")
      }
      tryCatch({
    ...
    } else if (identical(state, "stopped")) {
      run_location(paste(
        paste("Run folder:", run_dir_abs),
        "Archived outputs: not created because the run was stopped",
        "Files written before the stop remain in outputs/ until the next run.",
        sep = "\n"
      ))
      status("Stopped by user")
      bg_close_progress("Stopped")
      append_log("Run stopped by the user.")
    } else {
      run_location(paste(
        paste("Run folder:", run_dir_abs),
      ...
      status("Run failed")
      bg_close_progress("Failed")
      if (!is.null(st$err_msg) && nzchar(st$err_msg %||% "") &&
          !grepl(st$err_msg, logs(), fixed = TRUE)) {
        append_log(paste("ERROR:", st$err_msg))
      }
    }

    bg_run$active <- FALSE
    set_run_buttons(running = FALSE)
    invisible(NULL)
  }

  # Poll the background process about once a second while a run is active.
  observe({
    if (!isTRUE(bg_run$active)) return()
    invalidateLater(1000, session)
    run_dir <- isolate(bg_run$run_dir)
    if (is.null(run_dir)) return()
    ...
    if (state %in% c("finished", "failed", "stopped")) {
      finish_background_run(st)
      return()
    }

    # Progress bar and status line follow the phase reported by the driver.
    phase <- st$phase %||% NULL
    if (!is.null(phase) && !identical(phase, isolate(bg_run$phase))) {
      bg_run$phase <- phase
      if (!identical(phase, "Starting")) {
        bg_advance(phase, st$phase_detail %||% phase)
        status(sprintf("Running %s...", phase))
      }
    }

    # Stop requested: give the driver a moment to end cleanly, then end it.
    stop_at <- isolate(bg_run$stop_requested_at)
    if (!is.null(stop_at)) {
      waited <- as.numeric(difftime(Sys.time(), stop_at, units = "secs"))
      if (waited > 20) {
        sae_bg_stop(run_dir, kill_driver = TRUE)
        sae_bg_write_status(run_dir, state = "stopped", err_msg = "Run stopped by the user.")
        finish_background_run(sae_bg_read_status(run_dir))
      }
      return()
    }

    # The driver died without writing a final state (crash, killed from
    # outside): report a failure instead of waiting forever. The process
    # check spawns a system command, so do it every few seconds only.
    last_check <- isolate(bg_run$last_alive_check)
    if (!is.null(last_check) &&
        as.numeric(difftime(Sys.time(), last_check, units = "secs")) < 5) {
      return()
    }
    bg_run$last_alive_check <- Sys.time()
    pids <- sae_bg_read_pids(run_dir)
    elapsed <- as.numeric(difftime(Sys.time(), isolate(bg_run$started_at), units = "secs"))
    driver_pid <- if ("driver" %in% names(pids)) pids[["driver"]] else st$driver_pid
    if (!is.null(driver_pid)) {
      if (!sae_process_alive(driver_pid)) {
        sae_bg_write_status(run_dir, state = "failed",
          err_msg = paste("The analysis process ended unexpectedly. See",
                          file.path(run_dir, "driver_output.log"), "and run.log."))
        finish_background_run(sae_bg_read_status(run_dir))
      }
    } else if (elapsed > 60) {
      sae_bg_write_status(run_dir, state = "failed",
        err_msg = paste("The analysis process did not start. See",
                        file.path(run_dir, "driver_output.log")))
      finish_background_run(sae_bg_read_status(run_dir))
    }
  })

  observeEvent(input$stop_btn, {
    if (!isTRUE(isolate(bg_run$active))) {
      showNotification("No analysis is running.", type = "message", duration = 4)
      return()
    }
    if (!is.null(isolate(bg_run$stop_requested_at))) return()
    run_dir <- isolate(bg_run$run_dir)
    bg_run$stop_requested_at <- Sys.time()
    status("Stopping...")
    killed <- sae_bg_stop(run_dir)
    append_log(paste0(
      "Stop requested by the user",
      if (length(killed)) paste0(" (ending ", paste(killed, collapse = ", "), ")") else "",
      "..."
    ))
    showNotification("Stopping the analysis...", type = "warning", duration = 5)
  })
  ...
  observeEvent(input$run_btn, {
    # Only one analysis at a time: a second click while the background
    # process is running must not start another run (or restart this one).
    if (isTRUE(isolate(bg_run$active))) {
      showNotification("An analysis is already running. Press Stop to end it first.",
                       type = "warning", duration = 6)
      return()
    }
    ...
    # The bar is closed here only if the run does not start (validation
    # errors); once launched, the background-run observer closes it.
    launched <- FALSE
    on.exit(if (!launched) progress$close(), add = TRUE)
    step_counter <- 0L
    ...
    # ---- Step 2: launch the analysis in a background R process ----
    # scripts/run_pipeline_bg.R runs UFH / MFH / Comparison, the optional AI
    # interpretations, the report and the archive copy. This process only
    # polls run.log and run_status.rds (see the observer below), so the
    # dashboard stays responsive and the Stop button can end the run.
    ai_requested <- "Comparison" %in% input$steps && isTRUE(input$llm_enabled)
    ai_note_eligible <- ai_requested &&
    ...
    job <- list(
      config_path = normalizePath(cfg_path, winslash = "/", mustWork = TRUE),
      run_id      = run_id,
      steps       = input$steps,
      ai          = list(
        requested       = ai_requested,
        eligible        = ai_note_eligible,
        provider        = provider_hint,
        model           = model_hint,
        language        = get_language(),
        consent         = isTRUE(input$llm_external_consent),
        indicator_type  = input$indicator_type %||% "poverty",
        currency_symbol = input$currency_symbol %||% "EUR",
        # The Comparison report is MFH-driven, so the AI interpretation
        # must describe the MFH scale (see the note in the driver).
        log_transform   = identical(input$mfh_transformation, "log") &&
                          identical(input$indicator_type, "mean_welfare")
      )
    )
    saveRDS(job, file.path(run_dir, "job.rds"))

    launch_ok <- tryCatch({
      sae_bg_launch(run_dir, api_key = if (ai_note_eligible) input$api_key else NULL)
      TRUE
    }, error = function(e) {
      append_log(paste("ERROR: could not start the analysis process:", conditionMessage(e)))
      FALSE
    })
    if (!launch_ok) {
      status("Run failed")
      run_location(paste(
        paste("Run folder:", run_dir_abs),
        "Archived outputs: not created because the run could not start",
        sep = "\n"
      ))
    ...
    bg_run$active           <- TRUE
    bg_run$launched         <- TRUE
    bg_run$run_dir          <- run_dir
    bg_run$run_dir_abs      <- run_dir_abs
    bg_run$run_log_path     <- run_log_path
    bg_run$log_lines_read   <- 0L
    bg_run$progress         <- progress
    bg_run$step_counter     <- step_counter
    bg_run$n_steps          <- n_steps
    bg_run$phase            <- NULL
    bg_run$started_at       <- Sys.time()
    bg_run$stop_requested_at <- NULL
    bg_run$last_alive_check <- NULL
    bg_run$ctx <- list(
      do_benchmark           = isTRUE(input$do_benchmark),
      benchmark_source_label = benchmark_source_label,
      benchmark_level_label  = benchmark_level_label,
      benchmark_level_var    = benchmark_level_var,
      steps                  = input$steps,
      country                = if (nzchar(trimws(input$country_name %||% ""))) trimws(input$country_name) else "Not specified"
    )
    launched <- TRUE
    set_run_buttons(running = TRUE)
    append_log("Analysis started in a background process; you can follow the log here and press Stop to end it.")
  })
