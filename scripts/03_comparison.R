...
plot_poverty_map <- function(col_name, method_label, year_val) {
  ...
  ggplot(map_sf) +
    ...
    # Shared level scale (R/map_style.R): light = low rate, dark = high rate,
    # same value range on every poverty map (.map_level_limits).
    sae_fill_level(labels = label_number(accuracy = 0.01),
                   limits = if (exists(".map_level_limits")) .map_level_limits else NULL,
                   oob = scales::squish) +
...
plot_rmse_map <- function(col_name, method_label, year_val) {
  ...
  ggplot(map_sf) +
    geom_sf(aes(fill = value), color = NA) +
    sae_fill_level(labels = label_number(accuracy = 0.001),
                   limits = if (exists(".map_rmse_limits")) .map_rmse_limits else NULL,
                   oob = scales::squish) +
    labs(caption = sae_map_caption(.map_attribution), 
...
# One legend range for all poverty maps and one for all RMSE maps (every
# method, both years), so the same colour means the same value on each map.
.map_value_cols   <- vapply(.display_map_specs, function(sp) sp$col, character(1))
.map_level_limits <- sae_shared_limits(comparison_dt, .map_value_cols)
.map_rmse_limits  <- sae_shared_limits(comparison_dt, paste0(.map_value_cols, "_RMSE"))
...
# One legend range, centred on zero, for every change map (all methods):
