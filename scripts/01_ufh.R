...
.map_plot_eur <- function(pov_fh_year, shp, year_label, value_col = "FH_Bench",
  ...
  ggplot2::ggplot(shp_join) +
    ...
    sae_fill_level(name = "EUR", limits = limits, oob = scales::squish) +
...
# UFH level and RMSE maps drawn with the shared scheme (R/map_style.R):
# Direct and FH (benchmarked when benchmarking is on) side by side on one
# scale, and their RMSEs on a second figure. Replaces emdi::map_plot(),
# whose white-to-red gradient did not match the other maps.
.map_plot_fh_panels <- function(pov_fh_year, shp, year_label, benchmarked = FALSE,
                                est_limits = NULL, rmse_limits = NULL) {
  est_col <- if (isTRUE(benchmarked) && "FH_Bench" %in% names(pov_fh_year)) "FH_Bench" else "FH"
  est_lab <- if (identical(est_col, "FH_Bench")) "FH (benchmarked)" else "FH"
  panels <- pov_fh_year %>%
    transmute(
      domain      = trimws(as.character(domain)),
      Direct_est  = Direct,
      FH_est      = .data[[est_col]],
      Direct_rmse = sqrt(pmax(Direct_MSE, 0)),
      FH_rmse     = sqrt(pmax(.data[[paste0(est_col, "_MSE")]], 0))
    )
  build <- function(cols, labels, fill_name, fill_labels, limits = NULL) {
    long <- panels %>%
      select(domain, all_of(cols)) %>%
      tidyr::pivot_longer(-domain, names_to = "panel", values_to = "value") %>%
      mutate(panel = factor(labels[panel], levels = labels))
    map_sf <- shp %>%
      select(domain) %>%              # sf keeps the geometry column
      left_join(long, by = "domain")
    ggplot(map_sf) +
      geom_sf(aes(fill = value), color = NA) +
      facet_wrap(~ panel, nrow = 1) +
      sae_fill_level(name = fill_name, labels = fill_labels,
                     limits = limits, oob = scales::squish) +
      labs(caption = sae_map_caption(.map_attribution)) +
      sae_map_theme(base_size = 15) +
      theme(strip.text = element_text(size = 15, face = "bold"))
  }
  list(
    estimates = build(c("Direct_est", "FH_est"),
                      c(Direct_est = "Direct", FH_est = est_lab),
                      pov_lab$short, label_number(accuracy = 0.01), est_limits) +
      labs(title = paste0(pov_lab$short, " -- ", year_label)),
    rmse = build(c("Direct_rmse", "FH_rmse"),
                 c(Direct_rmse = "Direct RMSE", FH_rmse = paste(est_lab, "RMSE")),
                 "RMSE", label_number(accuracy = 0.001), rmse_limits) +
      labs(title = paste0("RMSE -- ", year_label))
  )
}
...
if (.use_eur_plots) {
  # Drawn together with the year-2 map (below) so both years share one range.
  invisible(NULL)
} else {
  .unmatched <- setdiff(trimws(as.character(shp_dt[["domain"]])),
                        trimws(as.character(res_y1$pov_fh$domain)))
  ...
  # The year-1 maps are drawn together with the year-2 maps (below), so that
  # both years share one legend range.
}
...
if (.use_eur_plots) {
  .eur_limits <- sae_shared_limits(
    dplyr::bind_rows(dplyr::select(res_y1$pov_fh, dplyr::any_of("FH_Bench")),
                     dplyr::select(res_y2$pov_fh, dplyr::any_of("FH_Bench"))),
    "FH_Bench"
  )
  .p_map_y1 <- .map_plot_eur(res_y1$pov_fh, shp_dt, years_keep[1], limits = .eur_limits)
  ggsave(here::here("outputs", "figures", "ufh_map_y1.png"),
         .p_map_y1, width = 10, height = 8, dpi = 150)
  .p_map_y2 <- .map_plot_eur(res_y2$pov_fh, shp_dt, years_keep[2], limits = .eur_limits)
  ggsave(here::here("outputs", "figures", "ufh_map_y2.png"),
         .p_map_y2, width = 10, height = 8, dpi = 150)
} else {
  .unmatched <- setdiff(trimws(as.character(shp_dt[["domain"]])),
                        trimws(as.character(res_y2$pov_fh$domain)))
  if (length(.unmatched) > 0) {
    stop(
  ...
  # One legend range for the estimates and one for the RMSEs across Direct,
  # FH and both years, so the same colour means the same value on every map.
  .ufh_map_bench <- isTRUE(do_benchmark) && !is.null(res_y1$fh_bench) && !is.null(res_y2$fh_bench)
  .ufh_map_est_col <- if (.ufh_map_bench) "FH_Bench" else "FH"
  .ufh_map_pool <- dplyr::bind_rows(
    dplyr::select(res_y1$pov_fh, dplyr::any_of(c("Direct", "Direct_MSE", .ufh_map_est_col, paste0(.ufh_map_est_col, "_MSE")))),
    dplyr::select(res_y2$pov_fh, dplyr::any_of(c("Direct", "Direct_MSE", .ufh_map_est_col, paste0(.ufh_map_est_col, "_MSE"))))
  )
  .ufh_map_pool$Direct_RMSE <- sqrt(pmax(.ufh_map_pool$Direct_MSE, 0))
  .ufh_map_pool$Model_RMSE  <- sqrt(pmax(.ufh_map_pool[[paste0(.ufh_map_est_col, "_MSE")]], 0))
  .ufh_map_est_limits  <- sae_shared_limits(.ufh_map_pool, c("Direct", .ufh_map_est_col))
  .ufh_map_rmse_limits <- sae_shared_limits(.ufh_map_pool, c("Direct_RMSE", "Model_RMSE"))
  for (.yi in 1:2) {
    .res_yi <- if (.yi == 1L) res_y1 else res_y2
    .p_maps <- .map_plot_fh_panels(.res_yi$pov_fh, shp_dt, years_keep[.yi],
                                   benchmarked = .ufh_map_bench,
                                   est_limits = .ufh_map_est_limits,
                                   rmse_limits = .ufh_map_rmse_limits)
    ggsave(here::here("outputs", "figures", sprintf("ufh_map_y%d.png", .yi)),
           .p_maps$estimates, width = 12, height = 6, dpi = 150)
    ggsave(here::here("outputs", "figures", sprintf("ufh_rmse_map_y%d.png", .yi)),
           .p_maps$rmse, width = 12, height = 6, dpi = 150)
  }
}
