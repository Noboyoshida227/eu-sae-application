...
- One colour scheme and one legend range for all maps of a kind (`R/map_style.R`): every level map (poverty rates, RMSE, mean welfare in EUR, the UFH and MFH level maps) uses the reversed magma scale, lighter = lower and darker = higher, with grey for domains without an estimate; every change map (poverty change, growth rates) uses the same blue-white-red diverging scale centred on zero. Previously each map stretched the colours over its own minimum and maximum, so the same colour meant different values on different maps; now all poverty maps share one value range (Direct, UFH, MFH, both years), all RMSE maps another, and all change maps a symmetric range around zero (`sae_shared_limits()`), so colours can be compared across methods and years. The UFH maps previously drawn by `emdi::map_plot()` (white-to-red) are now `ufh_map_y*.png` (Direct and FH estimates) and `ufh_rmse_map_y*.png` (their RMSEs) on that scale; the RMSE maps in the report switch from inferno to the shared scale. Report captions updated.
