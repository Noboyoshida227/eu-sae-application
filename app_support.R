...
.pipeline_run_step <- function(step, script, config_path, logger = message) {
  ...
  # When the pipeline runs under the app's background driver (SAE_RUN_DIR is
  # set), the child records its own PID so that the Stop button can end it.
  step_pid_file <- sae_bg_step_pid_file(step)
  writeLines(
    c(
      "options(warn = 1)",
      sprintf("Sys.setenv(SAE_APP_CONFIG = %s, SAE_APP_STEP = %s)",
              r_literal(config_path), r_literal(step)),
      if (!is.null(step_pid_file)) {
        sprintf("try(writeLines(as.character(Sys.getpid()), %s), silent = TRUE)",
                r_literal(step_pid_file))
      },
  ...
  if (!is.null(step_pid_file)) {
    dir.create(dirname(step_pid_file), recursive = TRUE, showWarnings = FALSE)
    on.exit(unlink(step_pid_file, force = TRUE), add = TRUE)
  }
...
# ============================================================
# Background run: the dashboard launches one analysis as a separate R
# process (scripts/run_pipeline_bg.R) so that the app stays responsive,
# the log can be followed live, and a run can be stopped. Base R only.
#
# Files inside the run folder (app_runs/<run_id>/):
#   job.rds           what the driver should do (config path, AI options)
#   run_status.rds    progress written by the driver, read by the app
#   run.log           human-readable log (driver + pipeline messages)
#   driver_output.log raw stdout/stderr of the driver process
#   pids/driver.pid   PID of the driver;  pids/step_<STEP>.pid  PID of the
#                     R child currently running that step
#   STOP              marker written when the user presses Stop
# ============================================================

sae_bg_run_dir <- function() {
  d <- Sys.getenv("SAE_RUN_DIR", "")
  if (nzchar(d)) d else NULL
}

sae_bg_status_path <- function(run_dir) file.path(run_dir, "run_status.rds")

sae_bg_read_status <- function(run_dir) {
  path <- sae_bg_status_path(run_dir)
  if (!file.exists(path)) return(NULL)
  tryCatch(readRDS(path), error = function(e) NULL)
}

# Merge `...` into the stored status and write it back. Written to a
# temporary file first so a reader never sees a half-written file.
sae_bg_write_status <- function(run_dir, ...) {
  current <- sae_bg_read_status(run_dir) %||% list()
  updates <- list(...)
  for (nm in names(updates)) current[[nm]] <- updates[[nm]]
  current$updated_at <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
  path <- sae_bg_status_path(run_dir)
  tmp <- paste0(path, ".tmp")
  ok <- tryCatch({
    saveRDS(current, tmp)
...
sae_bg_stop_marker <- function(run_dir) file.path(run_dir, "STOP")

sae_bg_stop_requested <- function(run_dir = sae_bg_run_dir()) {
  !is.null(run_dir) && file.exists(sae_bg_stop_marker(run_dir))
}

# Signalled (as an error of class "sae_run_stopped") when the user has
# pressed Stop; the driver turns it into the final state "stopped".
sae_bg_check_stop <- function(run_dir = sae_bg_run_dir(), logger = NULL) {
  if (!sae_bg_stop_requested(run_dir)) return(invisible(FALSE))
  if (is.function(logger)) logger("Stop requested by the user; the run ends here.")
  cond <- structure(
    class = c("sae_run_stopped", "error", "condition"),
    list(message = "Run stopped by the user.", call = NULL)
  )
  stop(cond)
}

sae_bg_pid_dir <- function(run_dir) file.path(run_dir, "pids")

sae_bg_step_pid_file <- function(step, run_dir = sae_bg_run_dir()) {
  if (is.null(run_dir)) return(NULL)
  file.path(sae_bg_pid_dir(run_dir), paste0("step_", step, ".pid"))
}

sae_bg_record_pid <- function(run_dir, name = "driver", pid = Sys.getpid()) {
  dir.create(sae_bg_pid_dir(run_dir), recursive = TRUE, showWarnings = FALSE)
  writeLines(as.character(pid), file.path(sae_bg_pid_dir(run_dir), paste0(name, ".pid")))
  invisible(pid)
}

# Named integer vector: driver = <pid>, step_UFH = <pid>, ...
sae_bg_read_pids <- function(run_dir) {
  files <- list.files(sae_bg_pid_dir(run_dir), pattern = "\\.pid$", full.names = TRUE)
  if (length(files) == 0) return(integer())
  pids <- vapply(files, function(f) {
    v <- suppressWarnings(as.integer(trimws(readLines(f, n = 1L, warn = FALSE))))
    if (length(v) == 1L && is.finite(v)) v else NA_integer_
  }, integer(1))
  names(pids) <- sub("\\.pid$", "", basename(files))
  pids[!is.na(pids)]
}

# Image (executable) name of a process, or "" when no such process exists.
sae_process_image <- function(pid) {
  pid <- suppressWarnings(as.integer(pid))
  if (length(pid) != 1L || !is.finite(pid) || pid <= 0) return("")
  out <- tryCatch({
    if (.Platform$OS.type == "windows") {
      suppressWarnings(system2("tasklist",
        c("/FI", shQuote(sprintf("PID eq %d", pid)), "/FO", "CSV", "/NH"),
        stdout = TRUE, stderr = TRUE))
    } else {
      suppressWarnings(system2("ps", c("-p", pid, "-o", "comm="),
                               stdout = TRUE, stderr = TRUE))
    }
  ...
  if (.Platform$OS.type == "windows") {
    line <- out[grepl(sprintf('"%d"', pid), out, fixed = TRUE)]
    if (length(line) == 0) return("")
    fields <- strsplit(line[[1]], '","', fixed = TRUE)[[1]]
    return(gsub('^"|"$', "", fields[[1]]))
  }
  basename(out[[1]])
}

sae_process_alive <- function(pid) nzchar(sae_process_image(pid))

sae_is_r_process <- function(pid) {
  grepl("^(R|Rscript|Rterm|R\\.exe|Rscript\\.exe|Rterm\\.exe)$",
        sae_process_image(pid), ignore.case = TRUE)
}

# Terminate a process, but only if it really is an R process: PIDs are
# reused by the operating system and a stale pid file must never end an
# unrelated program. Returns TRUE when a termination signal was sent.
sae_kill_r_process <- function(pid) {
  if (!sae_is_r_process(pid)) return(FALSE)
  isTRUE(tryCatch(tools::pskill(as.integer(pid), tools::SIGTERM),
                  error = function(e) FALSE))
}

# Ask a background run to stop: write the marker (the driver checks it
# between steps), end the R child of the current step, and optionally
# the driver itself (used when the driver does not react in time).
sae_bg_stop <- function(run_dir, kill_driver = FALSE) {
  writeLines(format(Sys.time(), "%Y-%m-%d %H:%M:%S"), sae_bg_stop_marker(run_dir))
  pids <- sae_bg_read_pids(run_dir)
  killed <- character()
  for (nm in names(pids)) {
    if (nm == "driver" && !isTRUE(kill_driver)) next
    if (sae_kill_r_process(pids[[nm]])) killed <- c(killed, nm)
  }
  invisible(killed)
}

# Launch scripts/run_pipeline_bg.R for a prepared run folder. The API key
# (if any) travels through the environment of the child process only; it
# is never written to disk. Returns the path of the driver's raw log.
sae_bg_launch <- function(run_dir, api_key = NULL) {
  rscript <- file.path(R.home("bin"), if (.Platform$OS.type == "windows") "Rscript.exe" else "Rscript")
  if (!file.exists(rscript)) rscript <- "Rscript"
  driver <- file.path("scripts", "run_pipeline_bg.R")
  if (!file.exists(driver)) stop("Background driver not found: ", driver, call. = FALSE)
  driver_log <- file.path(run_dir, "driver_output.log")
  unlink(c(sae_bg_stop_marker(run_dir), sae_bg_status_path(run_dir)), force = TRUE)
  unlink(list.files(sae_bg_pid_dir(run_dir), full.names = TRUE), force = TRUE)
  sae_bg_write_status(run_dir, state = "starting", phase = "Starting",
                      phase_detail = "Starting the analysis process")
  key <- if (is.character(api_key) && length(api_key) == 1L && nzchar(api_key)) api_key else ""
  old_key <- Sys.getenv("SAE_LLM_API_KEY", unset = NA)
  Sys.setenv(SAE_LLM_API_KEY = key)
  on.exit({
    if (is.na(old_key)) Sys.unsetenv("SAE_LLM_API_KEY") else Sys.setenv(SAE_LLM_API_KEY = old_key)
  }, add = TRUE)
  system2(
    command = rscript,
    args = c("--vanilla", shQuote(normalizePath(driver, winslash = "/", mustWork = TRUE)),
             shQuote(normalizePath(run_dir, winslash = "/", mustWork = TRUE))),
    stdout = driver_log,
    stderr = driver_log,
    wait = FALSE
  )
  invisible(driver_log)
}
...
run_pipeline_from_config <- function(config_path, logger = message,
  ...
  for (step in steps) {
    ...
    sae_bg_check_stop(logger = logger)
    if (is.function(progress_callback)) {
    ...
    # A step that was killed by the Stop button comes back with a non-zero
    # status; report it as a user stop, not as a failure.
    sae_bg_check_stop(logger = logger)
