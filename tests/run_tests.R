...
wizard_resources <- c(
  ...
  "docs/instructions/EU_SAE_Download_Instructions_5_2_0_rc_6_wizard_5_8.pdf",
  "docs/instructions/EU_SAE_User_Guide_5_2_0_rc_6_wizard_5_8.pptx"
)
...
driver_text <- read_all("scripts/run_pipeline_bg.R")
check(grepl("render_report     = FALSE", driver_text, fixed = TRUE) &&
        grepl("render_final_report(", driver_text, fixed = TRUE) &&
        !grepl("render_final_report(", app_text, fixed = TRUE),
      "background driver defers the single final report render until AI status is ready")
check(grepl("sae_bg_launch(run_dir", app_text, fixed = TRUE) &&
        grepl('observeEvent(input$stop_btn', app_text, fixed = TRUE) &&
        grepl('"stop_btn"', wizard_text, fixed = TRUE),
      "dashboard runs the analysis in a background process with a Stop button in both UIs")
...
check(grepl('logger(paste("ERROR:", err_msg))', driver_text, fixed = TRUE) &&
        grepl('finish("failed", ok = FALSE, err_msg = err_msg', driver_text, fixed = TRUE),
      "pipeline failures are persisted to the run log and the run status file")
root_anchor_specs <- c(
